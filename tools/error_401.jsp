...
<utility:setBundle basename="totp" templateName="TOTP" var="totp"/>
...
<body onload="document.loginForm.username.focus()" class="login">
    ...
    <h2 class="loginlogo"></h2>
		...
		<c:set var="askForTotp" value="false" />
        <ui:loginArea>
            <h3 class="loginIcon"><fmt:message key="label.login"/></h3>
            <br class="clearFloat" />
        <ui:isLoginError var="loginResult">
		  <c:choose> 
		  <c:when test="${loginResult == 'ask_totp'}">
			<c:set var="askForTotp" value="true" />
			<span class="error"><fmt:message key="label.ask.totp" bundle="${totp}"/></span>
			<table cellspacing="1" cellpadding="0" border="0" class="formTable">
            <tbody>
            <tr>
                <th><fmt:message key="label.totp" bundle="${totp}"/></th>
                <td><input type="text" value="" style="width: 150px;" tabindex="1" maxlength="250" size="13" name="totp"/></td>
            </tr>
            </tbody>
        </table>
		</c:when>
