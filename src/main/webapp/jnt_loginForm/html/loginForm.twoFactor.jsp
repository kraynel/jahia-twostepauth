...
<c:if test="${!renderContext.loggedIn || currentAliasUser.username eq 'guest'}">
    <ui:loginArea class="loginForm" onsubmit="loginButton.disabled = true; return true;">
        ...
        <c:choose>
        <c:when test="${not empty param['loginError'] && param['loginError'] == 'ask_totp'}">
			<c:set var="askForTotp" value="true" />
			<span class="error"><fmt:message key="label.ask.totp"/></span>
			
			 <p>
            	<label class="left" for="username"><fmt:message key="label.totp"/></label>
            	<input class="full" type="text" value="" tabindex="1" name="totp" id="totp"  autocomplete="off" />
        	</p>
                ...
                <c:choose>
                    <c:when test="${fn:contains(publicPropertiesAsString, 'j:picture')}">
                        ...
                        <c:if test="${empty picture}">
                            <img src="<c:url value='${url.templatesPath}/default/images/userbig.png'/>" alt="" border="0"/>
                        </c:if>
                    </c:when>
                    <c:otherwise>
                        <img src="<c:url value='${url.templatesPath}/default/images/userbig.png'/>" alt="" border="0"/>
